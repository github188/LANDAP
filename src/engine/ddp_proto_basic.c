...
)
{
    ...
    if (inHdr->ipVer == IPV4_FLAG) {
        ...
        outMsgLen = HDR_END_V6_OFFSET + DDP_REPLY_LEN_DISCOVERY;
    }
    /* allocate output message */
    outMsg = (UINT1*)malloc(outMsgLen);
    if (outMsg == NULL) {
    ...
    ddp_platform_get_field(ifs, DDP_FIELD_BRAND_NAME, outBody + pos, &len);
    pos += DDP_FIELD_LEN_BRAND_NAME; len = DDP_FIELD_LEN_PRODUCT_NAME;
    ddp_platform_get_field(ifs, DDP_FIELD_PRODUCT_NAME, outBody + pos, &len);
    pos += DDP_FIELD_LEN_PRODUCT_NAME; len = DDP_FIELD_LEN_MODEL_NAME;
    ddp_platform_get_field(ifs, DDP_FIELD_MODEL_NAME, outBody + pos, &len);
    pos += DDP_FIELD_LEN_MODEL_NAME; len = DDP_FIELD_LEN_VERSION;
    ddp_platform_get_field(ifs, DDP_FIELD_VERSION, outBody + pos, &len);
    pos += DDP_FIELD_LEN_VERSION; len = DDP_FIELD_LEN_PRODUCT_TYPE;
    tmpU2 = 0;
    ddp_platform_get_field(ifs, DDP_FIELD_PRODUCT_TYPE, (UINT1*)&tmpU2, &len);
    tmpU2 = DDP_HTONS(tmpU2);
    memcpy(outBody + pos, (INT1*)&tmpU2, sizeof(tmpU2));
    //ddp_platform_get_field(ifs, DDP_FIELD_PRODUCT_TYPE, outBody + pos, &len);
    pos += DDP_FIELD_LEN_PRODUCT_TYPE; len = DDP_FIELD_LEN_PRODUCT_HW_SPEC;
    tmpU4 = 0;
    ddp_platform_get_field(ifs, DDP_FIELD_PRODUCT_HW_SPEC, (UINT1*)&tmpU4, &len);
    tmpU4 = DDP_HTONL(tmpU4);
    memcpy(outBody + pos, (INT1*)&tmpU4, sizeof(tmpU4));
    //ddp_platform_get_field(ifs, DDP_FIELD_PRODUCT_HW_SPEC, outBody + pos, &len);
    pos += DDP_FIELD_LEN_PRODUCT_HW_SPEC; len = DDP_FIELD_LEN_MAC_ADDR;
    ddp_platform_get_field(ifs, DDP_FIELD_MAC_ADDR, outBody + pos, &len);
    pos += DDP_FIELD_LEN_MAC_ADDR; len = DDP_FIELD_LEN_SYSTEM_NAME;
    ddp_platform_get_field(ifs, DDP_FIELD_SYSTEM_NAME, outBody + pos, &len);
    pos += DDP_FIELD_LEN_SYSTEM_NAME; len = DDP_FIELD_LEN_DEVICE_IP_ADDR;
    ddp_platform_get_field(ifs, DDP_FIELD_DEVICE_IP_ADDR, outBody + pos, &len);
    pos += DDP_FIELD_LEN_DEVICE_IP_ADDR; len = DDP_FIELD_LEN_WEB_SERVICE_PORT;
    tmpU2 = 0;
    ddp_platform_get_field(ifs, DDP_FIELD_WEB_SERVICE_PORT, (UINT1*)&tmpU2, &len);
    tmpU2 = DDP_HTONS(tmpU2);
    memcpy(outBody + pos, (INT1*)&tmpU2, sizeof(tmpU2));
    //ddp_platform_get_field(ifs, DDP_FIELD_WEB_SERVICE_PORT, outBody + pos, &len);
    pos += DDP_FIELD_LEN_WEB_SERVICE_PORT + DDP_FIELD_LEN_RESERVED_1; len = DDP_FIELD_LEN_CUSTOMIZED_DNS;
    ddp_platform_get_field(ifs, DDP_FIELD_CUSTOMIZED_DNS, outBody + pos, &len);
    pos += DDP_FIELD_LEN_CUSTOMIZED_DNS; len = DDP_FIELD_LEN_SUBNET_MASK;
    ddp_platform_get_field(ifs, DDP_FIELD_SUBNET_MASK, outBody + pos, &len);
    pos += DDP_FIELD_LEN_SUBNET_MASK; len = DDP_FIELD_LEN_DEFAULT_GATEWAY;
    ddp_platform_get_field(ifs, DDP_FIELD_DEFAULT_GATEWAY, outBody + pos, &len);
    pos += DDP_FIELD_LEN_DEFAULT_GATEWAY; len = DDP_FIELD_LEN_PRIMARY_DNS;
    ddp_platform_get_field(ifs, DDP_FIELD_PRIMARY_DNS, outBody + pos, &len);
    pos += DDP_FIELD_LEN_PRIMARY_DNS; len = DDP_FIELD_LEN_SECONDARY_DNS;
    ddp_platform_get_field(ifs, DDP_FIELD_SECONDARY_DNS, outBody + pos, &len);
    pos += DDP_FIELD_LEN_SECONDARY_DNS; len = DDP_FIELD_LEN_DHCP;
    ddp_platform_get_field(ifs, DDP_FIELD_DHCP, outBody + pos, &len);
    pos += DDP_FIELD_LEN_DHCP; len = DDP_FIELD_LEN_DATE_TIME;
    ddp_platform_get_field(ifs, DDP_FIELD_DATE_TIME, outBody + pos, &len);
    pos += DDP_FIELD_LEN_DATE_TIME; len = DDP_FIELD_LEN_TIMEZONE;
    //tmpU2 = 0;
    //ddp_platform_get_field(ifs, DDP_FIELD_TIMEZONE, (UINT1*)&tmpU2, &len);
    //tmpU2 = DDP_HTONS(tmpU2);
    //memcpy(outBody + pos, (INT1*)&tmpU2, sizeof(tmpU2));
